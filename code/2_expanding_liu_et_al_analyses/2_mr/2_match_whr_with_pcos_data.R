...
all_variants <- fread("output/1_curated_data/whr_female_ivs.txt")

#Also the whole dataframe to get proxies if necessary:

exposure_df <- whr

proxies <- fread("output/2_replicating_liu_et_al/2_mr/0_proxies/whr_female_ivs_proxies.txt", fill = TRUE)

proxies <- proxies[which(proxies$query_snp_rsid%in%all_variants$variant),]

length(which(duplicated(proxies$query_snp_rsid) == FALSE)) #196
...
fwrite(proxies, "output/2_replicating_liu_et_al/2_mr/0_proxies/whr_proxies_df.txt")

proxies <- fread("output/2_replicating_liu_et_al/2_mr/0_proxies/whr_proxies_df.txt")
...
data_df <- whr[which(whr$variant%in%all_variants$variant),]
...
recursively_matching_data(exp_df = data_df, list_of_phenos_ =list_of_phenos, trait_names_ = trait_names, output_path <- output_path, exposure = "whr_liu_")
