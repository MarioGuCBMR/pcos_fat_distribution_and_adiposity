...
mr_plotter_first <- function(mr_df, title){
  ...
  plotio <- plotio + scale_y_continuous(limits = c(-2, 3))
...
bmi_res <- "output/2_replicating_liu_et_al/2_mr/2_mr/bmi/"
whr_res <- "output/2_replicating_liu_et_al/2_mr/2_mr/whr/"
whradjbmi_res <- "output/2_replicating_liu_et_al/2_mr/2_mr/whradjbmi/"
...
##########################################################################################
#Let's add the genetic correlations and save the data in the best formatted data possible#
##########################################################################################

bmi_gc <- gc_2_mr_df(doctor = c(0.3021 ,0.0407), 
                           broad = c(0.2066 ,0.0414),
                           consortium = c(0.2422 ,0.0314),
                           meta = c(0.4471 ,0.0449),
                           venkatesh=c(0.3586,0.0502),
                           adj_age = c(0.3079 ,0.0805), 
                           adj_age_bmi = c(0.1283 ,0.111),
                           amenorrhea = c(-0.1887, 0.1023),
                           oligomenorrhea = c(0.1996, 0.0725),
                           menorrhagia = c(0.17, 0.0262),
                           anovulation = c(0.0874, 0.0493),
                           hirsutism = c(0.3033, 0.0673))

whr_gc <- gc_2_mr_df(doctor = c(0.2749 ,0.0444), 
                           broad = c(0.221 ,0.0443),
                           consortium = c(0.1704 ,0.0285),
                           meta = c(0.3295 ,0.044), 
                           venkatesh=c(0.3024,0.0441),
                           adj_age = c(0.3586 ,0.0633), 
                           adj_age_bmi = c(0.3262 ,0.0869),
                           amenorrhea = c(0.0226, 0.107),
                           oligomenorrhea = c(0.1414, 0.0688),
                           menorrhagia = c(0.1707, 0.0265),
                           anovulation = c(0.1531, 0.0526),
                           hirsutism = c(0.1634, 0.0606))

whradjbmi_gc <- gc_2_mr_df(doctor = c(0.1297 ,0.0431), 
                      broad = c(0.124 ,0.0448),
                      consortium = c(0.0443 ,0.0293),
                      meta = c(0.1131 ,0.0433), 
                      venkatesh=c(0.1156,0.0416),
                      adj_age = c(0.216 ,0.0706), 
                      adj_age_bmi = c(0.2704 ,0.1005),
                      amenorrhea = c(0.155, 0.1083),
                      oligomenorrhea = c(0.0286, 0.0679),
                      menorrhagia = c(0.0881, 0.0291),
                      anovulation = c(0.1191, 0.0542),
                      hirsutism = c(0.0014, 0.0631))
...
bmi_df <- rbind(bmi_df, bmi_gc)
whr_df <- rbind(whr_df, whr_gc)
whradjbmi_df <- rbind(whradjbmi_df, whradjbmi_gc)

##########################################################################################
#Let's plot PCOS results and additional traits later, because the plot is a bit too messy# BMI first and see what is up#
##########################################################################################

no_vect=c("amenorrhea", "oligomenorrhea",  "menorrhagia", "anovulation", "hirsutism")

bmi_pcos_df <- bmi_df[which(!(bmi_df$outcome%in%no_vect)),]
whr_pcos_df <- whr_df[which(!(whr_df$outcome%in%no_vect)),]
whradjbmi_pcos_df <- whradjbmi_df[which(!(whradjbmi_df$outcome%in%no_vect)),]

bmi_pcos_plot=mr_plotter_first(bmi_pcos_df, "")
whr_pcos_plot=mr_plotter_mid(whr_pcos_df, "")
whradjbmi_pcos_plot=mr_plotter_last(whradjbmi_pcos_df, "")
...
ggsave(
  filename = "manuscript/figures/drafts/gc_mr_bmi_whr_whradjbmi_pcos.svg",
  plot = pcos_all,
  width = 15,
  height = 11,
...
#And now for the other traits - just to have them

bmi_others_df <- bmi_df[which(bmi_df$outcome%in%no_vect),]
whr_others_df <- whr_df[which(whr_df$outcome%in%no_vect),]
whradjbmi_others_df <- whradjbmi_df[which(whradjbmi_df$outcome%in%no_vect),]

bmi_others_plot=mr_plotter_first(bmi_others_df, "")
whr_others_plot=mr_plotter_mid(whr_others_df, "")
whradjbmi_others_plot=mr_plotter_last(whradjbmi_others_df, "")

library(patchwork)

others_all=bmi_others_plot+whr_others_plot+whradjbmi_others_plot+plot_layout(ncol=3)

ggsave(
  filename = "manuscript/figures/drafts/gc_mr_bmi_whr_whradjbmi_others.svg",
  plot = others_all,
  width = 15,
  height = 10,
  units = "in",  # or "in" depending on your preference, but cm is common
...
fwrite(bmi_df, "manuscript/supplementary_tables/drafts/supplementary_table_2_bmi_pcos.csv")
fwrite(whr_df, "manuscript/supplementary_tables/drafts/supplementary_table_2_whr_pcos.csv")
fwrite(whradjbmi_df, "manuscript/supplementary_tables/drafts/supplementary_table_2_whradjbmi_pcos.csv")
