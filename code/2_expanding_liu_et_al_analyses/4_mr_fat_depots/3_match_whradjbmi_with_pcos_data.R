...
#This code matches WHRadjBMI female IVs utilized by Liu et al with PCOS data from 6 distinct GWAS.
...
all_variants <- fread("output/1_curated_data/whradjbmi_female_ivs.txt")

#Also the whole dataframe to get proxies if necessary:

exposure_df <- whradjbmi

proxies <- fread("output/2_replicating_liu_et_al/2_mr/0_proxies/whradjbmi_female_ivs_proxies.txt", fill = TRUE)

proxies <- proxies[which(proxies$query_snp_rsid%in%all_variants$variant),]

length(which(duplicated(proxies$query_snp_rsid) == FALSE)) #256
...
fwrite(proxies, "output/2_replicating_liu_et_al/2_mr/0_proxies/whradjbmi_proxies_df.txt")

proxies <- fread("output/2_replicating_liu_et_al/2_mr/0_proxies/whradjbmi_proxies_df.txt")
...
data_df <- whradjbmi[which(whradjbmi$variant%in%all_variants$variant),]
...
recursively_matching_data(exp_df = data_df, list_of_phenos_ =list_of_phenos, trait_names_ = trait_names, output_path <- output_path, exposure = "whradjbmi_liu_")
