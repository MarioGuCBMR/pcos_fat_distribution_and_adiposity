...
vat <- fread("output/1_curated_data/vat_curated_saaket_female.txt")
all_variants <- fread("output/1_curated_data/vat_female_ivs.txt")

#Also the whole dataframe to get proxies if necessary:

exposure_df <- vat

proxies <- fread("output/2_replicating_liu_et_al/4_mr_fat_depots/0_proxies/vat_female_haplreg_output.txt", fill = TRUE)

proxies <- proxies[which(proxies$query_snp_rsid%in%all_variants$variant),]

length(which(duplicated(proxies$query_snp_rsid) == FALSE)) #2
...
fwrite(proxies, "output/2_replicating_liu_et_al/4_mr_fat_depots/0_proxies/vat_proxies_df.txt")

proxies <- fread("output/2_replicating_liu_et_al/4_mr_fat_depots/0_proxies/vat_proxies_df.txt")
...
data_df <- vat[which(vat$variant%in%all_variants$variant),]
...
recursively_matching_data(exp_df = data_df, list_of_phenos_ =list_of_phenos, trait_names_ = trait_names, output_path <- output_path, exposure = "vat_")
